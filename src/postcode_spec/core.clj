...
(s/def ::postcode-letter
  (s/with-gen
    char?
    (fn [] (gen/fmap (fn [char-code]
                       (char char-code))
                     (s/gen (s/int-in 65 91))))))

(s/def ::postcode-digit (s/int-in 0 10))

(s/def ::outward-code-district-digit-letter
  (s/with-gen
    (s/and seq? #(re-matches #"[0-9][A-Z]" (apply str %)))
    (fn [] (gen/fmap (fn [parts]
                       (list (:postcode-digit parts) (:postcode-letter parts)))
                     (s/gen (s/keys :req-un [::postcode-letter
                                             ::postcode-digit]))))
    ))

(s/def ::outward-code-area (s/coll-of ::postcode-letter  :min-count 1 :max-count 2))
(s/def ::outward-code-district (s/or :two-letters (s/coll-of ::postcode-digit :min-count 1 :max-count 2)
                                     :digit-then-letter ::outward-code-district-digit-letter))
(s/def ::postcode-outward-code (s/keys :req-un [::outward-code-area
                                                ::outward-code-district]))
...
(s/def ::postcode-inward-code (s/keys :req-un [::inward-code-sector
                                               ::inward-code-unit]))
...
(s/def ::united-kingdom-postcode
  (let [re #"[A-Z][A-HK-Y]?[0-9][A-Z0-9]? [0-9][A-Z]{2}|[G][I][R] 0[A]{2}"]
    (s/with-gen
      ...
      (fn [] (gen/fmap (fn [parts]
                       ...
                       (s/gen (s/keys :req-un [::postcode-outward-code
